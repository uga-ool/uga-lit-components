...
- `uga-video`: **Multiple videos on one page could fail to load.** `playerid` feeds directly into the Kaltura embed container's DOM id and defaults to the same fixed value when omitted, so two `<uga-video>` elements that both omitted `playerid` (or that were given the same explicit `playerid`) rendered container divs with identical ids — Kaltura's `setup()` threw "target id provided is already in use" for the second one, which then displayed "This video failed to load." Each instance now mixes a unique per-instance id into its container id, so `playerid` can be omitted or reused freely across elements.
...
- **`playerid` selects the Kaltura player again** — it is the **uiConf ID**, the number after `uiconf_id/` in the Kaltura embed code, and it is what carries program/college branding and automatic end cards. Default **`57494843`** ("UGA Online Lit Component Player") when the attribute is omitted. This reverses the previous entry below, which made `playerid` the container `<div>` id: that number is an arbitrary timestamp in Kaltura's embed snippet, not a player, so every video rendered in one hard-coded player and branded players were unreachable.
  - Course HTML carrying the retired default `playerid="660400380"` keeps working: it is not a valid uiConf ID (Kaltura returns 404), so it falls back to the default player with a console warning. Update those pages to a real uiConf ID, or drop the attribute.
  - Different players on one page each load their own player bundle (roughly 4-5 MB apiece), so prefer a single player per page unless the branding genuinely differs.
- **Removed the username/classlist video format.** A data file whose `data` was an object keyed by instructor username is no longer supported; only the flat array (`{"data": ["1_abc", "1_def"]}`) is. That path fetched the whole course roster to resolve a filename, and matched `RoleId === 195`, which is the *student* role elsewhere in this repo, so it was selecting the wrong users.
- **`ou` is honored.** An author-supplied `ou` attribute is no longer overwritten by the course ID parsed from the URL, so a nested `<uga-rating>` posts into the intended course.
- `videodata`, `loaded`, `videos` and `versions` are internal state and are no longer settable as attributes or properties.
- Fixed: moving an `<uga-video>` in the DOM (D2L content panes, tabs, slideshows) duplicated its video list and left an empty black player behind. Fixed: a page with several videos could fire a burst of identical `whoami` requests during playback, and could send duplicate topic-completion calls.
- **Previous behavior, now reversed:** `playerid` was the embed's player ID (the `kaltura_player_<N>` number), used as the container/`targetId`, with `partnerId` (1727411) and `uiConfId` (52620262) as fixed constants and a default player ID of `660400380`.
- Removed `entryTitle` from `KalturaPlayer.setup()`. Playkit 7.276 does not read it (the string appears nowhere in the player bundle), and populating it required two cross-origin Kaltura calls from the render path. The Kaltura-reported name is still fetched, but only when `includerating` is set, since `uga-rating` is its only real consumer.
